...
PACKAGES = ["config", "players", "countryballs", "info", "admin", "trade", "blacklist"]
...
class BallsDexBot(commands.AutoShardedBot):
    ...
    def __init__(self, command_prefix: str, dev: bool = False, **options):
        ...
        # emojis_and_stickers: DB holds emoji IDs for the maps which are fetched from 3 servers
    ...
    async def load_cache(self):
        ...
        for ball in await Ball.all():
            balls[ball.pk] = ball
        log.info(f"Loaded {len(balls)} maps")
