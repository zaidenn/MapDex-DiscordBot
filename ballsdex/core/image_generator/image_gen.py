...
title_font = ImageFont.truetype(str(SOURCES_PATH / "constanb.ttf"), 170)
...
def draw_card(ball_instance: "BallInstance"):
    ...
    draw.text((50, 40), ball.short_name or ball.country, font=title_font)
    for i, line in enumerate(textwrap.wrap(f"SKILL: {ball.capacity_name.upper()}", width=28)):
        draw.text(
            (100, 1050 + 100 * i),
            line,
    ...
    draw.text(
        ...
        "Created by El Laggron and modified by tezmeon.\n" f"Artwork author: {ball.credits}",
        font=credits_font,
        fill=(0, 0, 0, 255),
        stroke_width=0,
