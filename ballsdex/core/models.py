...
class GuildConfig(models.Model):
    ...
    spawn_channel = fields.BigIntField(
        description="Discord channel ID where maps will spawn", null=True
    )
    enabled = fields.BooleanField(
        description="Whether the bot will spawn maps in this guild", default=True
    )
...
class Ball(models.Model):
    ...
    catch_names = fields.TextField(
        null=True,
        default=None,
        description="Additional possible names for catching this map, separated by semicolons",
    )
    ...
    health = fields.IntField(description="Map health stat")
    attack = fields.IntField(description="Map attack stat")
    rarity = fields.FloatField(description="Rarity of this map")
    enabled = fields.BooleanField(default=True)
    tradeable = fields.BooleanField(default=True)
    emoji_id = fields.BigIntField(
        description="Emoji ID for this map", validators=[DiscordSnowflakeValidator()]
    )
    wild_card = fields.CharField(
        max_length=200, description="Image used when a new map spawns in the wild"
    )
    collection_card = fields.CharField(
        max_length=200, description="Image used when displaying maps"
    )
    credits = fields.CharField(max_length=64, description="Author of the collection artwork")
    capacity_name = fields.CharField(
        max_length=64, description="Name of the map's capacity"
    )
    capacity_description = fields.CharField(
        max_length=256, description="Description of the map's capacity"
    )
