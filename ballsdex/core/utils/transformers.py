...
class CachedBallInstance:
    """
    Used to compute the searchable terms for a map only once.
    """
...
class BallInstanceTransformer(app_commands.Transformer):
    ...
    async def autocomplete(
        ...
        t2 = time.time()
        log.debug(
            f"MapInstance autocomplete took {round((t2-t1)*1000)}ms, {len(choices)} results"
        )
        return choices

    async def transform(self, interaction: discord.Interaction, value: str) -> BallInstance | None:
        # in theory, the selected map should be in the cache
        ...
        try:
            try:
                ...
                try:
                    ...
                    await interaction.response.send_message(
                        "The map could not be found. Make sure to use the autocomplete "
        ...
        for ball in self.cache.balls:
            if value.lower() in ball.country.lower():
                ...
                if len(choices) == 25:
                    break
        t2 = time.time()
        log.debug(f"Map autocomplete took {round((t2-t1)*1000)}ms, {len(choices)} results")
        return choices

    async def transform(self, interaction: discord.Interaction, value: str) -> Ball | None:
        if not value:
            await interaction.response.send_message(
                "You need to use the autocomplete function for the map selection."
            )
            return None
        try:
