...
class Admin(commands.GroupCog):
    ...
    balls = app_commands.Group(
        name=settings.players_group_cog_name, description="Maps management"
    )
    ...
    async def rarity(self, interaction: discord.Interaction, chunked: bool = True):
        """
        Generate a list of maps ranked by rarity.

        Parameters
        ----------
        chunked: bool
            Group together maps with the same rarity.
        """
        text = ""
    ...
    async def cooldown(
        ...
        if delta < 600:
            informations.append(
                "The manager is less than 10 minutes old, maps cannot spawn at the moment."
            )
        if informations:
            embed.add_field(
    ...
    async def spawn(
        self,
        interaction: discord.Interaction,
        map: BallTransform | None = None,
        channel: discord.TextChannel | None = None,
    ):
        """
        Force spawn a random or specified map.

        Parameters
        ----------
        map: Ball | None
            The map you want to spawn. Random according to rarities if not specified.
        channel: discord.TextChannel | None
            The channel you want to spawn the map in. Current channel if not specified.
        """
    ...
    async def give(
        ...
        """
        Give the specified map to a player.

        Parameters
        ----------
        map: Ball
            The map you want to give.
        user: discord.User
            The user you want to give a map to.
        special: Special | None
            A special background to set.
        shiny: bool
            Whether the map will be shiny or not. Omit this to make it random.
...
(-20/+20%).
        ...
        await interaction.followup.send(
            f"`{map.country}` {settings.collectible_name} was successfully given to `{user}`.\n"
            f"Special: `{special.name if special else None}` • ATK:`{instance.attack_bonus:+d}` • "
            f"HP:`{instance.health_bonus:+d}` • Shiny: `{instance.shiny}`"
        )
        await log_action(
            f"{interaction.user} gave {settings.collectible_name} {map.country} to {user}. "
            f"Special={special.name if special else None} ATK={instance.attack_bonus:+d} "
            f"HP={instance.health_bonus:+d} shiny={instance.shiny}",
            self.bot,
        )
    ...
    @app_commands.checks.has_any_role(*settings.root_role_ids, *settings.admin_role_ids)
    async def balls_info(self, interaction: discord.Interaction, map_id: str):
        """
        ...
        ----------
        map_id: str
            The ID of the map you want to get information about.
        """
        try:
            ballIdConverted = int(map_id, 16)
        except ValueError:
            await interaction.response.send_message(
    ...
    @app_commands.checks.has_any_role(*settings.root_role_ids)
    async def balls_delete(self, interaction: discord.Interaction, map_id: str):
        """
        Delete a map.
        ...
        await ball.delete()
        await interaction.response.send_message(
            f"{settings.collectible_name.title()} {map_id} deleted.", ephemeral=True
        )
    ...
    async def balls_transfer(
        self, interaction: discord.Interaction, map_id: str, user: discord.User
    ):
        """
        Transfer a map to another user.
        ...
        map_id: str
            The ID of the map you want to get information about.
        user: discord.User
            The user you want to transfer the map to.
        """
        try:
            ballIdConverted = int(map_id, 16)
        except ValueError:
    ...
    async def balls_reset(
        ...
        """
        Reset a player's maps.

        Parameters
        ----------
        user: discord.User
            The user you want to reset the maps of.
        percentage: int | None
            The percentage of maps to delete, if not all. Used for sanctions.
        """
    ...
    async def balls_count(
        ...
        interaction: discord.Interaction,
        user: discord.User = None,
        map: BallTransform = None,
        shiny: bool = None,
        special: SpecialTransform = None,
    ):
        """
        Count the number of maps that a player has or how many exist in total.

        Parameters
        ----------
        user: discord.User
            The user you want to count the maps of.
        map: Ball
            The map you want to count.
        shiny: bool
            Whether the map is shiny or not.
        special: Special
            The special background of the map.
        """
        filters = {}
        if map:
            filters["ball"] = map
        ...
        country = f"{map.country} " if map else ""
