...
class Trade(commands.GroupCog):
    ...
    @app_commands.command()
    async def add(self, interaction: discord.Interaction, map: BallInstanceTransform):
        """
        Add a map to the ongoing trade.

        Parameters
        ----------
        map: BallInstance
            The map you want to add to your proposal
        """
        if not map:
            return
        if not map.countryball.tradeable:
            await interaction.response.send_message(
                "You cannot trade this map.", ephemeral=True
            )
            return
        ...
        if map.favorite:
            ...
            await interaction.followup.send(
                "This map is a favorite, are you sure you want to trade it?",
                view=view,
                ephemeral=True,
            )
            await view.wait()
        ...
        trader.proposal.append(map)
        await interaction.followup.send(
            f"{map.countryball.country} added.", ephemeral=True
        )

    @app_commands.command()
    async def remove(self, interaction: discord.Interaction, map: BallInstanceTransform):
        """
        Remove a map from what you proposed in the ongoing trade.

        Parameters
        ----------
        map: BallInstance
            The map you want to remove from your proposal
        """
        if not map:
            return
        ...
        if trader.locked:
            ...
            return
        if map not in trader.proposal:
            await interaction.response.send_message(
                f"That {settings.collectible_name} is not in your proposal.", ephemeral=True
            )
            return
        trader.proposal.remove(map)
        await interaction.response.send_message(
            f"{map.countryball.country} removed.", ephemeral=True
        )
