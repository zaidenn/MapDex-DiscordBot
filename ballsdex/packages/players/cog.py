...
class Players(commands.GroupCog, group_name=settings.players_group_cog_name):
    ...
    async def list(
        ...
        sort: SortingCHoices
            Choose how maps are sorted. Can be used to show duplicates.
        """
    ...
    async def completion(self, interaction: discord.Interaction, user: discord.User | None = None):
        """
        Show your current completion of the MapDex.
        ...
        # Filter disabled maps, they do not count towards progression
        ...
        if owned_countryballs:
            # Getting the list of emoji IDs from the IDs of the owned maps
            fill_fields(
    ...
    async def info(self, interaction: discord.Interaction, map: BallInstanceTransform):
        """
        Display info from a specific map.

        Parameters
        ----------
        map: BallInstance
            The map you want to inspect
        """
        if not map:
            return
        await interaction.response.defer(thinking=True)
        content, file = await map.prepare_for_message(interaction)
    ...
    async def last(self, interaction: discord.Interaction):
        """
        Display info of your last caught map.
        """
        await interaction.response.defer(thinking=True)
        try:
    ...
    @app_commands.command()
    async def favorite(self, interaction: discord.Interaction, map: BallInstanceTransform):
        """
        Set favorite map.
        ...
        ----------
        map: BallInstance
            The map you want to set/unset as favorite
        """
        if not map:
            return
        ...
        if not map.favorite:
            ...
            map.favorite = True  # type: ignore
            await map.save()
            emoji = self.bot.get_emoji(map.countryball.emoji_id) or ""
            await interaction.response.send_message(
                f"{emoji} `#{map.pk:0X}` {map.countryball.country} "
                f"is now a favorite {settings.collectible_name}!",
                ephemeral=True,
            )

        else:
            map.favorite = False  # type: ignore
            await map.save()
            emoji = self.bot.get_emoji(map.countryball.emoji_id) or ""
            await interaction.response.send_message(
                f"{emoji} `#{map.pk:0X}` {map.countryball.country} "
    ...
    async def donation_policy(
        ...
        """
        Change how you want to receive donations from /maps give
    ...
    async def give(
        ...
        interaction: discord.Interaction,
        user: discord.User,
        map: BallInstanceTransform,
    ):
        """
        Give a map to a user.
        ...
        map: BallInstance
            The map you're giving away
        """
        if not map:
            return
        if not map.countryball.tradeable:
            await interaction.response.send_message(
                "You cannot donate this map.", ephemeral=True
            )
            return
        if user.bot:
            await interaction.response.send_message("You cannot donate to bots.")
            return
        if map.id in self.bot.locked_balls:
            await interaction.response.send_message(
                "This map is currently locked for a trade. Please try again later."
            )
            return
        self.bot.locked_balls[map.id] = None
        new_player, _ = await Player.get_or_create(discord_id=user.id)
        old_player = map.player

        if new_player == old_player:
            await interaction.response.send_message(
                f"You cannot give a {settings.collectible_name} to yourself."
            )
            del self.bot.locked_balls[map.id]
            return
        if new_player.donation_policy == DonationPolicy.ALWAYS_DENY:
            ...
            await interaction.response.send_message(
                ...
                f"{map.description(include_emoji=True, bot=interaction.client)}!\n"
                "Do you accept this donation?",
                view=DonationRequest(self.bot, interaction, map, new_player),
            )
            return

        map.player = new_player
        map.trade_player = old_player
        map.favorite = False
        await map.save()

        await interaction.response.send_message(
            f"You just gave the {settings.collectible_name} "
            f"{map.description(short=True, include_emoji=True, bot=self.bot)} to "
            f"{user.mention}!"
        )
        del self.bot.locked_balls[map.id]
    ...
    async def rarity(self, interaction=discord.Interaction):
        """
        Show the MapDex rarity list (credit to GamingadlerHD)
        """
        ...
        for collectible in sorted_collectibles:
            ...
            entry = (f"{emote}{name}", f"Rarity: {rarity}")
            entries.append(entry)

        # This is the number of maps who are displayed at one page, 
