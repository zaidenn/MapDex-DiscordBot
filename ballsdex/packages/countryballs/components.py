...
class CountryballNamePrompt(Modal, title=f"Catch this {settings.collectible_name}!"):
    ...
    async def on_error(self, interaction: discord.Interaction, error: Exception, /) -> None:
        log.exception("An error occured in map catching prompt", exc_info=error)
        if interaction.response.is_done():
            await interaction.followup.send("An error occured with this map.")
        else:
            await interaction.response.send_message("An error occured with this map.")
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if self.name.value.lower().strip() in possible_names:
            ...
            special = ""
            if ball.shiny:
                special += f"✨ **Nice, this is a *shiny* {settings.collectible_name}!** ✨\n"
    ...
    async def catch_ball(
        ...
        if not shiny and population:
            ...
            # None is added representing the common map
